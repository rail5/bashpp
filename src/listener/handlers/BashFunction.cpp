...
void BashppListener::exitBashFunction(std::shared_ptr<AST::BashFunction> node) {
	bpp_assert(topmost_entity_is<bpp::bash_function>(), "Function context was not found in the entity stack");
	auto function_entity = std::static_pointer_cast<bpp::bash_function>(entity_stack.top());
	entity_stack.pop();
