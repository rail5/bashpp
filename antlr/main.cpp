		...
		if (ruleContext) {
			switch (ruleContext->getRuleIndex()) {
				case BashppParser::RuleProgram:
					program += build_program(tree->children[i], in_class, class_name, in_method, method_signature, in_constructor);
					break;
				...
				case BashppParser::RuleClass_definition: {
					if (in_class) {
						throw std::runtime_error("Cannot define a class within a class");
					}
					...
					if (used_symbols.find(class_symbol) != used_symbols.end()) {
						throw std::runtime_error("Symbol '" + class_symbol + "' already in use");
					}

					used_symbols.insert(class_symbol);

					if (protected_keywords.find(class_name) != protected_keywords.end()) {
						throw std::runtime_error("Cannot use keyword '" + class_name + "' as a class name");
					}
				...
				case BashppParser::RuleClass_body_statement: {
					if (!in_class) {
						throw std::runtime_error("Cannot define a class body statement outside of a class");
					}
						...
						switch (innerRuleContext->getRuleIndex()) {
							case BashppParser::RuleVariable_definition: {
								...
								if (protected_keywords.find(variable_name) != protected_keywords.end()) {
									throw std::runtime_error("Cannot use keyword '" + variable_name + "' as a variable name");
								}

								if (classes[class_name].variables.find(variable_name) != classes[class_name].variables.end()) {
									throw std::runtime_error("Variable '" + variable_name + "' already defined in class '" + class_name + "'");
								}
								...
								if (used_symbols.find(variable_symbol) != used_symbols.end()) {
									throw std::runtime_error("Symbol '" + variable_symbol + "' already in use");
								}
							...
							case BashppParser::RuleMethod_definition: {
								...
								if (protected_keywords.find(method_name) != protected_keywords.end()) {
									throw std::runtime_error("Cannot use keyword '" + method_name + "' as a method name");
								}
								...
								if (classes[class_name].methods.find(method_signature) != classes[class_name].methods.end()) {
									throw std::runtime_error("Method signature '" + method_signature + "' already defined in class '" + class_name + "'");
								}
								...
								if (used_symbols.find(method_symbol) != used_symbols.end()) {
									throw std::runtime_error("Symbol '" + method_symbol + "' already in use");
								}
								...
								program += build_program(innerRuleContext, in_class, class_name, in_method, method_name, in_constructor);
								break; }
				...
				case BashppParser::RuleParameter_list:
					if (!in_method && !in_constructor) {
						throw std::runtime_error("Cannot define a parameter list outside of a method or constructor");
					}
				...
				case BashppParser::RuleOther_statement:
					...
					if (in_method) {
						...
						classes[class_name].methods[method_signature].method_body += ruleContext->getText();
					} else if (in_constructor) {
						classes[class_name].constructors.back().constructor_body += ruleContext->getText();
					} else {
						program += ruleContext->getText();
					}
					break;
