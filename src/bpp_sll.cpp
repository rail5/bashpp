...
bool parseWithMode(BashppParser* parser, std::shared_ptr<BashppListener> listener, atn::PredictionMode mode) {
	parser->getInterpreter<atn::ParserATNSimulator>()->setPredictionMode(mode);
	...
	try {
		...
		if (mode == atn::PredictionMode::SLL) {
			throw e;
		}
		std::cerr << "Parse cancellation exception: " << e.what() << std::endl;
...
int main(int argc, char* argv[]) {
	...
	std::shared_ptr<BashppListener> listener = std::make_shared<BashppListener>();
	...
	bool success = false;
	try {
		success = parseWithMode(&parser, listener, atn::PredictionMode::SLL);
	} catch (const ParseCancellationException& e) {
	//	tokens.reset();
		parser.reset();
		success = parseWithMode(&parser, listener, atn::PredictionMode::LL);
	}
