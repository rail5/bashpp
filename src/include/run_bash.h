 ...
 * 
 * @param filename The path to the Bash script to execute
 * @param arguments The arguments to pass to the Bash script
 * @return int The exit code of the Bash script
 * @throws bpp::ErrorHandling::InternalError if fork() or waitpid() fails
 */
inline int run_bash(const std::string& filename, const std::vector<char*>& arguments) {
	...
	if (WIFEXITED(status)) {
		...
		// There is a "folklore" kind of rule that "most tools return 128 + signal number" in this case
