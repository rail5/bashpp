...
class EntityMap {
	...
	public:
		 ...
		 */
		void insert(FilePosition start, FilePosition end, const EntityType& entity) {
			auto it = tree.insert({start, end});
			it.node()->payload = entity;
		}

		EntityType find(FilePosition point) {
		 ...
		 */
		EntityType find(uint32_t line, uint32_t column) {
